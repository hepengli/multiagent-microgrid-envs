...
class Environment_IEEE34_DDPG(gym.Env):
    metadata = {
        ...
        for agent in self.policy_agents:
            ...
            if agent.action.ncats is not None:
                if isinstance(agent.action.ncats, list):
                    ...
                    if agent.action.ncats == 33:
                        u_action_space = spaces.Box(low=np.array([-16]), high=np.array([16]), dtype=np.float32)
                    else:
                        u_action_space = spaces.Box(low=np.array([0]), high=np.array([4]), dtype=np.float32)
                    action_shape += 1
                else:
