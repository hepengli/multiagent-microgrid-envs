...
class MicrogridEnv(gym.Env):
    ...
    def __init__(self, world, reset_callback=None, reward_callback=None,
        ...
        for agent in self.agents:
            ...
            if agent.d_ncats is not None:
                if isinstance(agent.d_ncats, list):
                    u_action_space = spaces.MultiDiscrete(agent.d_ncats)
                elif isinstance(agent.d_ncats, int):
                    u_action_space = spaces.Discrete(agent.d_ncats)
                else:
                    raise NotImplementedError()
                total_action_space.append(u_action_space)
            # continuous action space
            if agent.c_range is not None:
                low, high = agent.c_range
    ...
    def step(self, action_n):
        ...
        for i, agent in enumerate(self.agents):
            t_index += agent.dim_c + agent.dim_d
            self._set_action(action_n[s_index:t_index], agent)
            s_index = t_index
        ...
        for i, agent in enumerate(self.agents):
            ...
            info_n.update({'r{}'.format(i): reward_n[-1]})
        ...
        if self.post_step_callback is not None:
            self.post_step_callback(self.world, self.np_random)

        return obs_n, reward_n, done_n, info_n

    def reset(self):
        # reset world
        self.reset_callback(self.world, self.np_random)
        # reset renderer
        self._reset_render()
    ...
    def _set_action(self, action, agent, time=None):
        ...
        # continuous actions
        if agent.dim_c > 0:
            agent.action.c = action[index:(index+agent.dim_c)]
            index += agent.dim_c 
        # discrete actions
        if agent.dim_d > 0:
            agent.action.d = action[index:(index+agent.dim_d)]
            index += agent.dim_d
    ...
    def render(self, mode='human', close=False):
        ...
        for i in range(len(self.viewers)):
            ...
            for e, entity in enumerate(self.world.entities):
                ...
                if 'agent' in entity.name:
                    self.render_geoms[e].set_color(*entity.color, alpha=alpha*0.5)
                    if not entity.silent:
